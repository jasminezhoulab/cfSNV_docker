if [ "$1" == "-h" ]; then
	echo ""
	echo "This function maps raw reads from FASTQ (gzip or not) files to the reference genome."
	echo ""
	echo "Usage: ./getbam_align.sh -f1 \${fastq1} -f2 \${fastq2} -g \${genome} -d \${database} -i \${id} -o \${output}
...
Rscript --vanilla getbam_align.R $fastq1 $fastq2 $genome $database $id $output
